...
const DEFAULT_PENDING_COLOR = { label: '#b45309', bg: '#fffbeb', text: '#92400e' };
const DEFAULT_ACCEPTED_COLOR = { label: '#16a34a', bg: '#f0fdf4', text: '#166534' };
const DEFAULT_EXPIRED_COLOR = { label: '#ef4444', bg: '#fef2f2', text: '#991b1b' };

export default function PasswordResetPanel({
  isOpen,
  onClose,
  cancelColor = 'var(--button-red, #ef4444)',
  pendingColor = DEFAULT_PENDING_COLOR,
  acceptedColor = DEFAULT_ACCEPTED_COLOR,
  ...
  const resolvedPendingColor = { ...DEFAULT_PENDING_COLOR, ...pendingColor };
  const resolvedAcceptedColor = { ...DEFAULT_ACCEPTED_COLOR, ...acceptedColor };
  const resolvedExpiredColor = { ...DEFAULT_EXPIRED_COLOR, ...expiredColor };
                        ...
                        return (
                          <li key={idx} style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: '6px', padding: '10px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            <div>
                              ...
                              <div style={{
                                fontSize: '0.75rem',
                                color: isExpired ? resolvedExpiredColor.label : item.status === 'Accepted' ? resolvedAcceptedColor.label : resolvedPendingColor.label,
                                marginBottom: '6px'
                              }}>
                              ...
                              {item.expiracion && (
                                <DateTag
                                  ...
                                  bgColor={isExpired ? resolvedExpiredColor.bg : item.status === 'Accepted' ? resolvedAcceptedColor.bg : resolvedPendingColor.bg}
                                  textColor={isExpired ? resolvedExpiredColor.text : item.status === 'Accepted' ? resolvedAcceptedColor.text : resolvedPendingColor.text}
                                />
                              )}
                            </div>
                            <Button
